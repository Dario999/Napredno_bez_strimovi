...
final class IntegerArray{
    ...
    public IntegerArray getSorted(){
        ...
        IntegerArray toReturn = new IntegerArray(newNiza);
        return toReturn;
    }

    public IntegerArray concat(IntegerArray ia){
        int niza2[] = ia.getNiza();
        int totalLength = niza2.length + this.length();
        int newNiza[] = new int[totalLength];
        for (int i = 0; i < this.length(); i++) {
            newNiza[i] = this.niza[i];
        }
        int counter = this.length();
        for (int i = 0; i < niza2.length; i++) {
            newNiza[counter] = niza2[i];
            counter++;
        }
        IntegerArray toReturn = new IntegerArray(newNiza);
        return toReturn;
    ...
    public String toString() {
        ...
        st.append(niza[this.length()-1] + "]");
        return st.toString();
    }

    @Override
    public boolean equals(Object obj) {
        IntegerArray temp = (IntegerArray)obj;
        int niza1[] = temp.getNiza();
        if (this.length() != niza1.length){
            return false;
        }
        for (int i = 0; i < this.length(); i++) {
            if(this.niza[i] != niza1[i]){
                return false;
            }
        }
        return true;
...
final class ArrayReader{
    ...
    public static IntegerArray readIntegerArray(InputStream input){
        ...
        int niza[] = new int[n];
        for (int i = 0; i < n; i++) {
            niza[i] = Integer.parseInt(st.nextToken());
        }
